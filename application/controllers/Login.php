...
class Login extends CI_Controller {
    ...
    public function index($modulo = NULL) {
        ...
        #if ($modulo != 'pet' && $modulo != 'odonto' && $modulo != 'medico' && $modulo != 'profissional' && $modulo != 'login')
        #    show_404();
        
        if (!$modulo && !$_SESSION['log']['modulo']) {
            $modulo = 'pet';
        }
        ...
        if ($_SESSION['log']['modulo'] == 'pet')
            $_SESSION['log']['cliente'] = $_SESSION['log']['modulo'];
        elseif ($_SESSION['log']['modulo'] == 'odonto' || $_SESSION['log']['modulo'] == 'medico')
            $_SESSION['log']['cliente'] = 'paciente';
        else
            $_SESSION['log']['cliente'] = 'cliente';
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if ($query === FALSE) {
                ...
                #$_SESSION['log']['NomeModulo'] = $modulo;
